...
const ModalAgregarOrden = ({ isOpen, onClose, handleSaveOrder }: { isOpen: boolean, onClose: () => void, handleSaveOrder: (order: any) => void }) => {
...
export default function DashboardTecnico() {
  type EstadoOrden = "Pendiente" | "Iniciado" | "Finalizado";
    ...
    const [orders, setOrders] = useState<{ date: string; id: string; device: string; status: string }[]>([]);
    ...
    return (
        <div className="max-w-[768px] min-h-screen text-white p-4 sm:p-8 max-w-screen-lg mx-auto mt-16">
            ...
            <div className="gap-16 p-4 pb-8">
                <section className="bg-white text-black p-4 rounded-lg my-4">
                    <h3 className="title1 text-primary-500 border-b border-primary-900 pb-2 flex items-center justify-between">
                        Ordenes
                        {usuario.rol === "Admin" && (
                            <button
                                onClick={() => setIsModalOpen(true)} // Abrir modal de agregar orden
                                className="ml-4 bg-primary-500 text-white px-3 py-1 rounded-lg text-sm"
                            >
                                Agregar Orden
                            </button>
                        )}
